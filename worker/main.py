...
import src.jobs.schemas as job_schemas
import src.ratings.schemas as rating_schemas
import src.resumes.schemas as resume_schemas
from src import constants as src_constants
from src.jobs import JobService
from src.ratings.service import RatingService
from src.resumes.service import ResumeService
...
async def get_abi() -> dict:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(src_constants.ABI_URL) as response:
        ...
        match event["event"]:
            ...
            case (
                ...
                schema = job_schemas.JobUpdateSchema(
                    **event["args"],
                    status=worker_constants.EVENT_TO_STATUS[event["event"]],
                )
...
async def listen_to_events():
    ...
    async with connect(worker_constants.WEBSOCKET_URL) as ws:
        await ws.send(
            json.dumps(
                {
                    "jsonrpc": "2.0",
